...
def main():
    alphabet = ''.join([chr(i) for i in range(65,91)]) #nagybetus abc
    required = alphabet + alphabet.lower() + '\'' + '123456789 '#aminek meg kell maradni a szovegbol
    exception = set([ch for ch in TEXT if ch not in required]) #a kitorolni kive+ant karakterek
    n_text = TEXT
    for ch in exception:
        n_text = n_text.replace(ch, '') #a karakterek kitorlese
    words = n_text.lower().split(' ') #felbontas stb...
    szotar = {}
    for s in words:
        if s not in szotar:
            szotar[s] = 1
